...
function getCountSql($sql) {
	...
	//return $sql;
	//var_dump(preg_match("/(select) (.*) (from .*)/i",$sql));exit('s');
	preg_match("/(select) (.*) (from .*)/i",$sql,$m);
	$sql = "select count(*) as count {$m[3]}";
	return $sql;
	//return preg_replace("/(select) (.*) (from .*)/i","\$1 count(*) as count \$3",$sql); //MB,linux上的nginx环境里的php失效
}


//补充字符变成定长
function paddingString($string, $char, $num, $direction = true){
	$n = $num - strlen($string);
	//echo strlen($string);
	if($n > 0){
		$char = createSameChar($char,$n);
		if($direction){ //前面补
			return $char.$string;
		}else{ //后面补
			return $string.$char;
...
//产生相同字符 str_repeat() 系统自带的
function createSameChar($char, $n){
	$str = '';
	for($i = 0; $i < $n; $i++){
		$str .= $char;
	}
