<?php
@ini_set('display_errors', 1); //显示报告
error_reporting(E_ALL ^ E_NOTICE); //所有错误
ini_set("memory_limit",1048576000); //更改内存限制为1G
...
if(file_exists('/data/httpd/2144/sync.2144.cn/channel/project.qq61/sources/stat/')){ //预发布环境
...
//			exit;
		...
		if(preg_match_all("/<table cellpadding=\"0\" cellspacing=\"0\" class=\"result\" id=\"(\d+)\".*><tr><td class=f><h3 class=\"t\">(<font.*?<\/font>)?<a.*?href=\"(.*?)\"\s*target=\"_blank\">(.*?)<\/a>\s*?<\/h3><font size=\-1>.*?<span class=\"g\">.*? ((\d{4}\-\d{1,2}\-\d{1,2})|(\d+小时前)|(\d+分钟前)) .*?<\/span>.*?<br><\/font><\/td><\/tr><\/table>/i", $contents, $out_all))
		{
			$jj = 0;
			foreach ($out_all[0] as $o_key => $o_val)
			{
				...
				if (preg_match("/:\/\/(\w*?\.)*?{$d}\//i", $o_href, $o_domain))
				{
					...
					$isrank = 1;
					$t[$jj]['k']      = function_change_utf8($k);   //关键字
					$t[$jj]['order']  = $o_id; //排行
					$t[$jj]['title']  = function_change_utf8($o_title);   //标题
					$t[$jj]['weight'] = $dm_weight;  //权重
					$t[$jj]['time']   = $o_cache;    //时间
					$jj++;
				}
			}
		}
