...
*/
	...
	RegNames	=	['Email','Phone','Require','Mobile','Number','Zip','QQ','Url','English','Chinese','Username'];
	var RegArray = {
		Require : /.+/,
		Email : /^(\w+([-+.]\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*)*$/,
	...
	function CheckForm(oForm,target)
	{
		...
		for(var i=0;i<els.length;i++)
		{
			...
			els[i].onblur= function(){
				...
				{
					//取得验证的正则字符串
					//prompt("this.check",this.check);
					var sReg = Filter(trim(this.check));
					//取得表单的值,用通用取值函数
					var sVal = GetValue(this);
					//字符串->正则表达式,不区分大小写
					var reg = new RegExp(sReg,"i");
					//prompt('',sVal);
					//prompt('',sReg);
					//prompt('',reg.test(sVal));
					if(!reg.test(sVal))
					{
						//验证不通过,弹出提示warning
						//this.styles.border = '1pt solid orange';
						if (target==undefined)
						{
							if(tar = this.getAttribute("target")){
								document.getElementById(tar).innerHTML = this.warning;
							}else{
								if(errorBox = this.nextSibling){
								
									errorBox.innerHTML = '<span style="font-weight:bold;color:red">'+this.warning+'</span>';
									currentErr  = errorBox;
								}else{
									alert(this.warning);
								}
							...
							$(target).innerHTML	=	'<div style="font-weight:bold;color:red">'+this.warning+'</div>';
