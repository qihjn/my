...
function setGroupActions($groupId,$actionIdList)
{
	...
	function setGroupUsers($groupId,$userIdList)
	{
		...
		$rs = $this->execute('INSERT INTO '.$this->tablePrefix.'role_user (role_id,user_id) SELECT a.id, b.id FROM '.$this->tablePrefix.'role a, '.C('RBAC_USER').' b WHERE '.$where);  //此处出现的$this->tablePrefix.'user b 限死了只能使用think_user表
		//echo 'INSERT INTO '.$this->tablePrefix.'role_user (role_id,user_id) SELECT a.id, b.id FROM '.$this->tablePrefix.'role a, '.C('RBAC_USER').' b WHERE '.$where;exit;
		if($result===false) {
			return false;
		}else {
			return true;
