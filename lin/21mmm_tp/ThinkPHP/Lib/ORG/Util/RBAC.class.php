...
class RBAC extends Think
{
    ...
    static public function authenticate($map,$model='')
    {
		...
		//var_dump(M($model)->getLastSql());exit;
        //使用给定的Map进行认证
        return D($model)->where($map)->find();
    }

    //用于检测用户权限的方法,并保存到Session中
    static function saveAccessList($authId=null,$roleName='')
    {
      
	   if(null===$authId)   $authId = $_SESSION[C('USER_AUTH_KEY')];
        // 如果使用普通权限模式，保存当前用户的访问权限列表
        // 对管理员开发所有权限
		//ri($_SESSION[C('ADMIN_AUTH_KEY')]);
        if(C('USER_AUTH_TYPE') !=2 && !$_SESSION[C('ADMIN_AUTH_KEY')] ){
            if($roleName){
				$_SESSION['_ACCESS_LIST']	=	RBAC::getRoleAccessList($roleName);
			}else{
	...
	//得到角色的访问列表
	static public function getRoleAccessList($roleName,$roleId=''){
		// echo $roleName;
		// Db方式权限数据
        $db     =   Db::getInstance();
        $table = array('role'=>C('RBAC_ROLE_TABLE'),'user'=>C('RBAC_USER_TABLE'),'access'=>C('RBAC_ACCESS_TABLE'),'node'=>C('RBAC_NODE_TABLE'));
		if(!$roleId && $roleName){
			$r = $db->query("select id from {$table['role']} where ename='$roleName'");
			$roleId = $r[0]['id'];
		}
        $sql    =   "select node.id,node.name from ".
                    $table['role']." as role,".
                    $table['access']." as access ,".
                    $table['node']." as node ".
                    "where '{$roleId}'=role.id and ( access.role_id='{$roleId}'  or (access.role_id=role.pid and role.pid!=0 ) ) and role.status=1 and access.node_id=node.id and node.level=1 and node.status=1";
        $apps =   $db->query($sql);//echo $sql;
        $access =  array();//dump($apps);
        foreach($apps as $key=>$app) {
            $appId	=	$app['id'];
            $appName	 =	 $app['name'];
            ...
            $sql    =   "select node.id,node.name from ".
                    ...
                    $table['access']." as access ,".
                    $table['node']." as node ".
                    "where  '{$roleId}'=role.id and ( access.role_id=role.id  or (access.role_id=role.pid and role.pid!=0 ) ) and role.status=1 and access.node_id=node.id and node.level=2 and node.pid={$appId} and node.status=1";
            $modules =   $db->query($sql);
            // 判断是否存在公共模块的权限
            $publicAction  = array();
            foreach($modules as $key=>$module) {
                ...
                $sql    =   "select node.id,node.name from ".
                    ...
                    $table['access']." as access ,".
                    $table['node']." as node ".
                    "where '{$roleId}'=role.id and ( access.role_id=role.id  or (access.role_id=role.pid and role.pid!=0 ) ) and role.status=1 and access.node_id=node.id and node.level=3 and node.pid={$moduleId} and node.status=1";
                    $rs =   $db->query($sql);
                    foreach ($rs as $a){
                        $publicAction[$a['name']]	 =	 $a['id'];
                    }
