...
class CommentModel extends CommonModel {
	...
	public function insert(){
		...
		if($id){
			$result = true;
			$user = D('user');
			$u = $user->getUserInfo($vo['touid']);
			//发送邮件
			$title = "恭喜，收到一份{$vo['name']}的供求咨询";
			$body = file_get_contents('./Public/Email/supply.html');
			$body = str_replace('{$toname}',$u['LXname'],$body);
			$body = str_replace('{$fromname}',$vo['name'],$body);
			$body = str_replace('{$content}',msubstr($vo['content'],0,100),$body);
			$body = str_replace('{$tid}',$vo['tid'],$body);
			$body = str_replace('{$date}',date('Y-m-d',time()),$body);
