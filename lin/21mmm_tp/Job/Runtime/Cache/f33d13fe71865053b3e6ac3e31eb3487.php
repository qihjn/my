...
.job_name a{ font-size:20px; color:#000;}
.job_detail{ padding:10px 10px 0px 25px;}
.magin_10{ margin-bottom:8px;}

-->
</style>

				<div class="ad"><img src="__PUBLIC__/Skin/Job/ad_4.jpg" width="930" border="0"/></div>
				<div class="left" id="jleft">
					<div class="login">
						...
						<?php if(is_array($up)): $i = 0; $__LIST__ = $up;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): ++$i;$mod = ($i % 2 )?><div class='swck'>
							<h2><span class='dx'><?php echo jurl2($vo['id'],$vo['jtitle'],'_blank');?></span></h2>
							<p><a href='<?php echo (curl($vo["nid"])); ?>' target='_blank'><?php echo ($vo["ctitle"]); ?></a></p>
							<p><?php echo ($vo["workplace"]); ?></p>
							<p><?php echo (date('Y-m-d',$vo["ctime"])); ?></p>
						</div><?php endforeach; endif; else: echo "" ;endif; ?>
					</div>
				</div>
				<div class="zhigou" id="jright">
					<div class="zjms" style="padding-top:10px;">
						<style>
							.zhigou .zjms .gw * {vertical-align:middle;}
						</style>
						                    <form action='/job/index/' method=get id="fso">
                        <!--<tr>
...
if((/sotype\/job/).exec(ref)){
    ...
    <div class="job_name">
        <input type="checkbox" id="id[]" name="id[]" value="<?php echo ($vo["id"]); ?>" />
        <a href="/job/show_<?php echo ($vo["id"]); ?>.html" target=_blank style="font-size:18px;color:#0c63e4"><?php echo ($vo["jtitle"]); ?></a> </div>
    <div class="job_detail">
        <div class="magin_10">
            <div>公司名称: <a href="/company/show_<?php echo ($vo["nid"]); ?>.html" target="_blank" style="font-size:13px;color:#f80"> <?php echo ($vo["ctitle"]); ?> </a> <span style=" margin-left:30px; color:#f80;">发布日期：<?php echo (date('Y-m-d',$vo["ctime"])); ?></span> </div>
            
        </div>
        <div class="magin_10"  style="color:#2A2A2A;"> <span class="bold">工作地点</span>:<?php echo ($vo["workplace"]); ?>  | <span class="bold">所属行业</span>:<?php echo (getEnumTitle($vo["category_id"],profesion)); ?> |  <span class="bold">公司性质</span>:<?php echo ($vo["nature"]); ?> |  <span class="bold">学历要求</span>:<?php echo ($vo["study"]); ?></div>
        <div> 职位要求:
            <?php echo ($vo["request"]); ?> </div>
    </div>
</div><?php endforeach; endif; else: echo "" ;endif; ?>
                        <div id="page_bottom" class="bg_fa">
									
										<a onclick="chkall('id[]')" style='cursor:pointer;'>全选</a> <a onclick="chkallno('id[]')" style='cursor:pointer;'> 取消全选</a>
									  <a onclick="apply();">申请此职位</a> <a onclick="fav();">放入收藏夹</a><!-- 每页显示：<select name=""><option>10条</option></select> <select name=""><option>按时间发布顺序排列</option></select>
										</span>									
						--><?php echo ($page); ?></div>  
...
<script>
//$('#page_bottom').style.display='none';
...
window.onload = function(){
	...
	//G("jleft").style.height="1588px";
	//同步左右div高度
	//equalHight("jleft","jright");
}
