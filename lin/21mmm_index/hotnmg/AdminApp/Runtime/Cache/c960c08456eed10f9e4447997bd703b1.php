...
*/
	...
	<form method="post" action="<?php echo U('Link/doCommand');?>">
	<div class="body_content">
    <div class="top_action"><a href="<?php echo U('Link/insert');?>">录入链接</a></div>
	<table cellspacing="0" cellpadding="0" id="maintable" class="formtable">
		...
		<td><input name="title[]" id="title[]" value="<?php echo ($vo["title"]); ?>" size="20" >
		  <span style="text-align:left"><?php echo (statusIcon($vo["status"],1,$frontUrl,'hidden.gif','隐藏')); ?></span><span style="text-align:left"><?php echo (attachStatus($vo["link_type"],'image',$frontUrl,'image.gif','图片')); ?> </span></td>
		<td><input name="link_url[]" id="link_url[]" value="<?php echo ($vo["link_url"]); ?>" class="input-style"/></td>
		<td><input name="display_order[]" id="display_order[]" value="<?php echo ($vo["display_order"]); ?>" size="3"/></td>
		<td><?php echo (date("Y-m-d",$vo["create_time"])); ?></td>
		<td><a href="<?php echo U('Link/modify',array('id'=>$vo['id']));?>"><img src="__PUBLIC__/Admin/modify22px.gif" alt="编辑" align="absmiddle" /></a></td>
