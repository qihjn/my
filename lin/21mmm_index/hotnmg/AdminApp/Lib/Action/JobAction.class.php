 ...
 * @version        	$Id: JobAction.class.php v2.0 2010-01-01 06:59:03 shuguang $
...
class JobAction extends GlobalAction
{
    ...
    public function resume()
    {
        ...
        $listRows = empty($pageSize) || $pageSize> 100 ? 15 : $pageSize ;
        $p = new page($count, $listRows);
        $dataList = $this->dao->Table(C('DB_PREFIX').'resume a')->Join(C('DB_PREFIX').'job b on a.job_id=b.id')->Field('a.*,b.title')->order($setOrder)->Where($condition)->Limit($p->firstRow.','.$p->listRows)->findAll();
        $page = $p->show();
        if($dataList !== false){
            $this->assign('dataList', $dataList);
            $this->assign('pageBar', $page);
