...
class ProductAction extends GlobalAction
{
    ...
    public function index()
    {
        ...
        $dataList = $this->dao->Table(C('DB_PREFIX').'product a')->Join(C('DB_PREFIX').'category b on a.category_id=b.id')->Field('a.*,b.title as category')->Order($setOrder)->Where($condition)->Limit($p->firstRow.','.$p->listRows)->findAll();
        $page = $p->show();
        if($dataList !== false)
        {
