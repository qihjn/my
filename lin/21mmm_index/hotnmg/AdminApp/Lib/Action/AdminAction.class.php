 ...
 * @version        	$Id: AdminAction.class.php v2.0 2010-01-01 06:59:03 shuguang $
...
class AdminAction extends GlobalAction
{
	...
	public function index()
	{
		...
		$dataList = $this->dao->Table(C('DB_PREFIX').'admin a')->Join(C('DB_PREFIX').'admin_role b on a.role_id=b.id')->Field('a.*,b.role_name as role_name')->Order($setOrder)->Where($condition)->Limit($p->firstRow.','.$p->listRows)->findAll();
		$page = $p->show();
		if($dataList!=false){
	...
	public function insert(){
		parent::_checkPermission();
		$roleList = M('AdminRole')->Order("id DESC")->findall();
